...
const Add = () => {
  ...
  const handleAddFood = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://54.158.145.141:9090/expenses/updateFood?id=${userId}`, {
        method: 'PUT',
  ...
  const handleAddEntertainment = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://54.158.145.141:9090/expenses/updateEntertainment?id=${userId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddUtility = async (e) => {
    ...
    try {
        const response = await fetch(`http://54.158.145.141:9090/expenses/updateUtility?id=${userId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
