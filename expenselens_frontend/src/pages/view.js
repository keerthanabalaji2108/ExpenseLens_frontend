...
const View = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const spendingResponse = await fetch(`http://18.204.215.150:9090/expenses/getSpendingVsSaving/${userId}`);
        const spendingData = await spendingResponse.json();
        setSpendingVsSaving(spendingData);

        const entertainmentResponse = await fetch(`http://18.204.215.150:9090/expenses/getEntertainment/${userId}`);
        const entertainmentData = await entertainmentResponse.json();
        setEntertainment(entertainmentData);

        const foodResponse = await fetch(`http://18.204.215.150:9090/expenses/getFood/${userId}`);
        const foodData = await foodResponse.json();
        setFood(foodData);

        const utilityResponse = await fetch(`http://18.204.215.150:9090/expenses/getUtility/${userId}`);
        const utilityData = await utilityResponse.json();
        setUtility(utilityData);

        const percentageResponse = await fetch(`http://18.204.215.150:9090/expenses/getAsPercentage/${userId}`);
